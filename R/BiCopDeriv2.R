...
#' object \code{obj}, the alternative version \cr \preformatted{BiCopDeriv2(u1,
#' u2, obj, deriv = "par")} can be used.
#'
...
#' @param deriv Derivative argument \cr \code{"par"} = second derivative with
#' respect to the first parameter (default)\cr \code{"par2"} = second
#' derivative with respect to the second parameter (only available for the
#' t-copula) \cr \code{"u1"} = second derivative with respect to the first
#' argument \code{u1} \cr \code{"u2"} = second derivative with respect to the
#' second argument \code{u2} \cr \code{"par1par2"} = second derivative with
#' respect to the first and second parameter (only available for the t-copula)
#' \cr \code{"par1u1"} = second derivative with respect to the first parameter
#' and the first argument \cr \code{"par2u1"} = second derivative with respect
#' to the second parameter and the first argument (only available for the
#' t-copula) \cr \code{"par1u2"} = second derivative with respect to the first
#' parameter and the second argument \cr \code{"par2u2"} = second derivative
#' with respect to the second parameter and the second argument (only available
#' for the t-copula) \cr
...
#' \itemize{ \item of the copula \code{family} \item with parameter(s)
#' \code{par}, \code{par2} \item with respect to \code{deriv} \item evaluated at
#' \code{u1} and \code{u2}. }
