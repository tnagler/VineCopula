...
#' \preformatted{BiCopHfuncDeriv2(u1, u2, obj, deriv = "par")} can be used.
#'
...
#' t-copula) \cr \code{"u2"} = second derivative with respect to the second
#' argument \code{u2} \cr \code{"par1par2"} = second derivative with respect to
#' the first and second parameter (only available for the t-copula) \cr
#' \code{"par1u2"} = second derivative with respect to the first parameter and
#' the second argument \cr \code{"par2u2"} = second derivative with respect to
#' the second parameter and the second argument (only available for the
#' t-copula) \cr
...
#' care).
#' @return A numeric vector of the second-order conditional bivariate copula
#' derivative \itemize{ \item of the copula \code{family} \item with parameter(s)
