...
#' Prokhorov (2011) for copulas. \cr Schepsmeier (2012) enhanced their approach
#' to the vine copula case. \cr The main contribution is that under correct
...
#' score function. The null hypothesis is \deqn{ }{H_0: H(\theta) + C(\theta) =
#' 0 }\deqn{ H_0: \boldsymbol{H}(\theta) + \boldsymbol{C}(\theta) = 0 }{H_0:
#' H(\theta) + C(\theta) = 0 } against the alternative \deqn{ }{H_1: H(\theta)
#' + C(\theta) != 0 }\deqn{ H_1: \boldsymbol{H}(\theta) +
#' \boldsymbol{C}(\theta) \neq 0 , }{H_1: H(\theta) + C(\theta) != 0 } where
...
#' score function. \cr For the calculation of the test statistic we use the
#' consistent maximum likelihood estimator \eqn{\hat{\theta}} and the sample
#' counter parts of \eqn{\boldsymbol{H}(\theta)}{H(\theta)} and
#' \eqn{\boldsymbol{C}(\theta)}{C(\theta)}. \cr The correction of the
...
#' 
#' \code{method = "IR"}: \cr As the White test the information matrix ratio
#' test is based on the expected Hessian matrix
#' \eqn{\boldsymbol{H}(\theta)}{H(\theta)} and the expected outer product of
#' the score function \eqn{\boldsymbol{C}(\theta)}{C(\theta)}. \cr \deqn{
#' }{H_0: -H(\theta)^{-1}C(\theta) = I_p}\deqn{ H_0:
#' -\boldsymbol{H}(\theta)^{-1}\boldsymbol{C}(\theta) = I_{p} }{H_0:
#' -H(\theta)^{-1}C(\theta) = I_p} against the alternative \deqn{ }{H_1:
#' -H(\theta)^{-1}C(\theta) != I_p.}\deqn{ H_1:
#' -\boldsymbol{H}(\theta)^{-1}\boldsymbol{C}(\theta) \neq I_{p} . }{H_1:
#' -H(\theta)^{-1}C(\theta) != I_p.} The test statistic can then be calculated
#' as \deqn{ IR_n:=tr(\Phi(\theta))/p } with
...
#' \eqn{tr(A)} is the trace of the matrix \eqn{A} \cr For details see
#' Schepsmeier (2013) \cr
#' 
#' \code{method = "Breymann"}, \code{method = "Berg"} and \code{method =
#' "Berg2"}: \cr These tests are based on the multivariate probability integral
...
#' s_t=\sum_{i=1}^d \Gamma(y_{it}), t=1,...,n } In Breymann et al. (2003) the
#' weight function is suggested as
...
#' 
#' \code{method = "ECP"} and \code{method = "ECP2"}: \cr Both tests are test
...
#' dimension p. They are based on the empirical copula process (ECP) \deqn{
#' \hat{C}_n(u)-C_{\hat{\theta}_n}(u), } with
#' \eqn{u=(u_1,\ldots,u_d)\in[0,1]^d} and \eqn{\hat{C}_n(u) =
#' \frac{1}{n+1}\sum_{t=1}^n \boldsymbol{1}_{\{U_{t1}\leq u_1,\ldots,U_{td}\leq
#' u_d \}} }{\hat{C}_n(u) = 1/(n+1)\sum_{t=1}^n 1_{U_{t1}\leq
#' u_1,\ldots,U_{td}\leq u_d} }. The ECP is utilized in a multivariate
...
#' null hypothesis. \cr Only the following copula families are allowed in
#' \code{RVM$family} due to restrictions in \code{\link{RVineGrad}} and
#' \code{\link{RVineHessian}} \cr \code{0} = independence copula \cr \code{1} =
...
#' \code{6} = Joe copula \cr \code{13} = rotated Clayton copula (180 degrees;
#' ``survival Clayton'') \cr \code{14} = rotated Gumbel copula (180 degrees;
#' ``survival Gumbel'') \cr \code{16} = rotated Joe copula (180 degrees;
#' ``survival Joe'') \cr \code{23} = rotated Clayton copula (90 degrees) \cr
#' \code{24} = rotated Gumbel copula (90 degrees) \cr \code{26} = rotated Joe
...
#' equality (default) \cr \code{"IR"} = goodness-of-fit test based on the
#' information ratio \cr \code{"Breymann"} = goodness-of-fit test based on the
#' probability integral transform (PIT) and the aggregation to univariate data
#' by Breymann et al. (2003). \cr \code{"Berg"} = goodness-of-fit test based on
#' the probability integral transform (PIT) and the aggregation to univariate
#' data by Berg and Bakken (2007). \cr \code{"Berg2"} = second goodness-of-fit
#' test based on the probability integral transform (PIT) and the aggregation
#' to univariate data by Berg and Bakken (2007). \cr \code{"ECP"} =
...
#' 200})\cr For \code{B = 0} the asymptotic p-value is returned if available,
#' otherwise only the test statistic is returned.\cr WARNING: If \code{B} is
#' chosen too large, computations will take very long.
#' @param alpha an integer of the set \code{2,4,6,...} for the \code{"Berg2"}
#' goodness-of-fit test (default \code{alpha = 2})
#' @return For \code{method = "White"}: \item{White}{test statistic}
#' \item{p.value}{p-value, either asymptotic for \code{B = 0} or bootstrapped
#' for \code{B > 0}} For \code{method = "IR"}: \item{IR}{test statistic}
...
#' Schepsmeier (2013)} For \code{method = "Breymann"}, \code{method = "Berg"}
#' and \code{method = "Berg2"}: \item{CvM, KS, AD}{test statistic according to
#' the choice of \code{statistic}} \item{p.value}{p-value, either asymptotic
...
#' \code{ADGofTest} is loaded. \cr Furthermore, a asymptotic p-value can be
#' calculated for the Kolmogorov-Smirnov test statistic. For the Cramer-von
#' Mises no asymptotic p-value is available so far.} For \code{method = "ECP"}
#' and \code{method = "ECP2"}: \item{CvM, KS}{test statistic according to the
#' choice of \code{statistic}} \item{p.value}{bootstrapped p-value}
